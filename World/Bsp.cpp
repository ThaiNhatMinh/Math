...
BSP::BSP(const char* fp)
{
	...
	//			printf("Prop name: %s\n", info.name);
	//			info.origin = Vec3(pStaticPropLumps[j].Origin.x, pStaticPropLumps[j].Origin.y, pStaticPropLumps[j].Origin.z);
	//			info.angles = Vec3(pStaticPropLumps[j].Angles.x, pStaticPropLumps[j].Angles.y, pStaticPropLumps[j].Angles.z);
...
void BSP::LoadMapDetails(Camera* cam)
{
	...
	Vec3 position(GetCoordsFromString(origin));
	Vec3 orientation(GetCoordsFromString(angles));
...
void BSP::LoadBSP(int node, /*int leaf,*/ Vec3 pos)
{
	if (node < 0)
	{
		if (node == -1)
...
void BSP::LoadFace(int face)
{
    ...
    // normal
    Vec3 vNormal = planes[faces[face].planeIndex].normal;
    if (faces[face].side)
        vNormal = -vNormal;
...
int BSP::WalkBSPTree(Vec3 pos, int node)
...
void BSP::LoadProps() 
{
	for (size_t i = 0; i < propInfo.size(); i++)
	{
		...
		mat4 modelMatrix;
		modelMatrix.Translate(propInfo[i].origin);
		printf("angles: %f %f %f\n", propInfo[i].angles.x, propInfo[i].angles.y, propInfo[i].angles.z);
		//modelMatrix = glm::rotate(modelMatrix, glm::radians(m_propInfo[i].angles.z), Vec3(1,0,0));
		//modelMatrix = glm::rotate(modelMatrix, glm::radians(m_propInfo[i].angles.x), Vec3(0,1,0));
		//modelMatrix = glm::rotate(modelMatrix, glm::radians(m_propInfo[i].angles.y), Vec3(0,0,1));
...
void BSP::RenderProps(int uniformModel, int uniformColor)
{
	for (size_t i = 0; i < propInfo.size(); i++)
	{
		...
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, propInfo.matrix.ToFloatPtr());
...
void BSP::LoadBrushEntities()
{
	for (int i = 0; i < nBrushEntities; i++)
	{
		...
		Vec3 origin = GetCoordsFromString(szOrigin);
		
		info.origin = origin;
		info.matrix.Translate(origin);
...
void BSP::RenderBrushEntities(int uniformModel, int uniformColor, int useUserColorUniform)
{
	for (int i = 0; i < nBrushEntities; i++)
	{
		...
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, info.matrix.ToFloatPtr());
...
Vec3 BSP::GetCoordsFromString(const char* str)
{
    if (!str)
    {
...
bool BSP::PointIsInside(Vec3 pos, short min[3], short max[3])
{
...
float BSP::DotProduct(Vec3 a, Vec3 b)
{
	return  (a.x * b.x + a.y * b.y + a.z * b.z);
}

void BSP::CrossProduct(const Vec3 v1, const Vec3 v2, Vec3& cross)
{
