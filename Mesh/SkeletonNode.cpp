...
void SkeAnim::ReadData(FILE * pFile, AnimNode& node,const vector<AnimKeyFrame>& KeyFrames)
{
...
FrameData SkeAnim::InterpolateFrame(unsigned int & currentFrame, const AnimNode & Anim, const vector<AnimKeyFrame>& KeyFrames)
{
	int frame0 = 0;
	int frame1 = -1;
	float t = 0.0f;
...
bool SkeAnim::LoadAnimation(FILE * pFile, const vector<SkeNode*>& pNodeList)
{
	...
	for (uint32 i = 0; i < CalcNumParentAnims; i++)
	{
		...
		m_pAnimList.push_back(p);
	}
}

void SkeAnim::Update(float dt, JointList& skeleton)
{
	if (!m_pAnimList.size()) return;
	m_fTime += dt;

	m_iCurrentFrame = (GLuint)(m_fTime * 1000);
	//cout << m_iCurrentFrame << ' '<< m_fTime<< endl;
	skeleton.clear();
	Animation* anim = m_pAnimList[m_iCurrentAnim];
	for (GLuint i = 0; i < anim->AnimNodeLists.size(); i++)
	{
		FrameData frame = InterpolateFrame(m_iCurrentFrame, anim->AnimNodeLists[i],anim->KeyFrames);
